...
5. [MLOps Practices](#mlops-practices)
...
- `source_URL`: Google Drive URL with dataset
- `local_data_file`: Where to save the downloaded ZIP
- `unzip_dir`: Where to extract the data
...
**Why VGG16?**
- Well-established architecture for image classification
- Good balance between accuracy and efficiency
- Pre-trained weights available in PyTorch

**Configuration** (`params.yaml`):
- `IMAGE_SIZE`: [224, 224, 3] - Standard input size
- `CLASSES`: 2 (Normal, Tumor)
- `LEARNING_RATE`: 0.001
- `WEIGHTS`: imagenet
...
**Why DVC?**
- Large files (images, models) don't fit in Git
- Need to track dataset versions
- Ensure reproducibility
- Share models and data efficiently
...
**Integration Points**:
1. Training: Log hyperparameters and metrics
2. Evaluation: Save model and metrics
3. Deployment: Load registered models
...
- **Usage**: Open in browser to interact with the model
...
- **Function**: Runs `python main.py` to train model
...
**Note**: This may take several minutes depending on hardware.
...
- Invalid image → 500 error
- Wrong format → 500 error
...
**Purpose**: Useful for checking model availability before prediction
...
```
- Save the URI (e.g., `123456789.dkr.ecr.us-east-1.amazonaws.com/kidney-classifier`)
...
##### 6. Configure Self-Hosted Runner
- Go to GitHub → Settings → Actions → Runners
- Add self-hosted runner on EC2
...
#### Issue 5: MLflow Tracking Not Working
**Error**: MLflow connection issues

**Solution**:
1. Check MLflow server is running: `mlflow ui`
2. Verify tracking URI in code
3. Check firewall/network settings
4. Use local tracking: `file://./mlruns`

#### Issue 6: Import Errors
**Error**: `ModuleNotFoundError`

**Solution**:
1. Activate conda environment: `conda activate cnncls`
2. Install package: `pip install -e .`
3. Check PYTHONPATH: `export PYTHONPATH=$PWD`
4. Verify `src/cnnClassifier/__init__.py` exists
...
4. Try rebuilding: `docker build --no-cache -t kidney-classifier .`
...
3. Use different port in `app.py`
...
1. **Model Architecture**: Try ResNet, EfficientNet, or Vision Transformers
2. **Data**: Collect more diverse datasets
3. **Augmentation**: Add more augmentation techniques
4. **Monitoring**: Implement model drift detection
5. **A/B Testing**: Compare different models
6. **API Versioning**: Support multiple model versions
7. **Autoscaling**: Auto-scale based on load
8. **Caching**: Cache predictions for frequently used images
